...
public class RUISTracker : MonoBehaviour 
{
	...
	public void OculusCounterPoseOffset()
	{
		if(coordinateSystem)
		{
			...
			{
				this.transform.localPosition = coordinateSystem.GetOculusRiftOrientation() * positionOffsetOculus + coordinateSystem.ConvertLocation
					(Vector3.zero, RUISDevice.Oculus_DK2);
			}
		}
	}
				...
				if (skeletonManager)
				{
					   ...
					   &&  jointData != null)
					{
						...
						hydraBasePositionCovariance = hydraBasePositionCovarianceKinect 
							+ Mathf.Clamp01(1.0f - 
							                jointData.positionConfidence)*2000;
			...
			else 
			{
				...
				if (compassData != null && compassData.rotationConfidence >= 1.0f) 
				{
					...
					updateDifferenceKalman( (compassData.rotation 
					                         * Quaternion.Inverse(Quaternion.Euler
					                     (compassRotationOffsetKinect))).eulerAngles, 
			...
			if (inputManager)
			{
				...
				if(compassPSMove)
				{
					...
					updateDifferenceKalman( (compassPSMove.localRotation 
					                         * Quaternion.Inverse(Quaternion.Euler
					                     (compassRotationOffsetPSMove))).eulerAngles, 
					                       driftingEuler, deltaT 				 );
				}
			}
			break;
			...
			if(compassRazer != null && compassRazer.Enabled)
			{
				...
				if(isRazerBaseMobile)
					updateDifferenceKalman((hydraBaseRotation * compassRazer.Rotation 
					                        * Quaternion.Inverse(Quaternion.Euler
					                     (compassRotationOffsetHydra))).eulerAngles,
					                       driftingEuler, deltaT 				 				
					                       );
				else
